...
function Account() {
    ...
    return (
        <>
            <div className="sticky top-0 z-50 flex items-center gap-2 px-2 py-3 bg-white dark:bg-black dark:text-white sm:px-4">
            ...
            <h3 className="mx-2 my-2 font-bold sm:mx-4 dark:text-white">My Profile</h3>
            <p className="mx-2 my-2 text-xs text-gray-400 sm:mx-4">Log in or Register to view your complete profile</p>

            <div className="flex items-center justify-center gap-2 my-3 font-bold text-green-700 dark:text-green-500">
            ...
            <div className="m-2 text-sm text-[#cbd5e0] md:mx-4 dark:text-gray-400">Legal & Support</div>

            <AccountMenu menu={accountMenu2} />

            <div className="m-2 text-sm text-[#cbd5e0] md:mx-4 dark:text-gray-400">Mobile Web Version 6.1.8</div>
        </>
    );
}
...
function AccountMenu({ menu }) {
    ...
    const menuItems = menu.map((item, index) => {
        return (
            <Link to={item.link} className="flex items-center justify-between px-2 py-3 border-b dark:border-gray-600" key={index}>
    ...
    return (
        <div className="mx-2 text-[#8693a6] shadow text-sm font-medium rounded-md md:mx-4 dark:shadow-gray-500">
            {menuItems}
        </div>
    )
}
